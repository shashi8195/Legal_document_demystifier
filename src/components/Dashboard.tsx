...
export const Dashboard: React.FC<DashboardProps> = ({ document, onViewDocument, language }) => {
...
Risk Level: ${document.analysis.riskLevel.toUpperCase()}

EXECUTIVE SUMMARY:
${document.analysis.summary}

KEY POINTS:
${document.analysis.keyPoints.map((point: string, index: number) => `${index + 1}. ${point}`).join('\n')}

RISKY CLAUSES:
${document.analysis.riskyClausesDetailed.map((clause: any, index: number) => `
...
YOUR RIGHTS:
${document.analysis.rights.map((right: any, index: number) => `
${index + 1}. ${right.title}
...
Generated by LegalClarify - AI-Powered Legal Document Analysis
    ...
    element.download = `${document.name}_analysis_summary.txt`;
...
${document.analysis.checklist.map((item: any, index: number) => `
☐ ${item.title}
...
Generated by LegalClarify - Your Legal Document Assistant
    ...
    element.href = URL.createObjectURL(file);
    element.download = `${document.name}_checklist.txt`;
    document.body.appendChild(element);
    element.click();
    document.body.removeChild(element);
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="flex items-start space-x-4">
            ...
            <div>
              ...
              <p className="text-gray-600 mt-1">
                {getLocalizedText('uploaded_on', language)} {new Date(document.uploadDate).toLocaleDateString()}
              </p>
              <div className="flex items-center mt-2 space-x-4">
                <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${getRiskColor(document.analysis?.riskLevel || 'low')}`}>
                  {getRiskIcon(document.analysis?.riskLevel || 'low')}
                  <span className="ml-2 capitalize">
                    {getLocalizedText('risk_level', language)}: {document.analysis?.riskLevel || 'Low'}
                  </span>
                </span>
      ...
      {/* Risk Analysis */}
      {document.analysis?.riskyClausesDetailed && document.analysis.riskyClausesDetailed.length > 0 && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <h2 className="text-xl font-semibold text-gray-900 mb-4">
            {getLocalizedText('risk_analysis', language)}
          </h2>
          <div className="space-y-4">
            {document.analysis.riskyClausesDetailed.slice(0, 3).map((clause: any, index: number) => (
      ...
      {document.analysis?.keyPoints && document.analysis.keyPoints.length > 0 && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <ul className="space-y-3">
            {document.analysis.keyPoints.map((point: string, index: number) => (
